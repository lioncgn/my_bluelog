...
class BlogTestCase(BaseTestCase):
    ...
    def test_new_admin_comment(self):
        response = self.client.post(url_for('blog.show_post', post_id=1), data=dict(body='I am admin comment.', post=
            Post.query.get(1)), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_new_admin_reply(self):
        response = self.client.post(url_for('blog.show_post', post_id=1) + '?reply=1', data=dict(body='I am an admin reply \
            comment.', post=Post.query.get(1)), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Comment published', data)
    ...
    def test_new_guest_reply(self):
        self.logout()
        response = self.client.post(url_for('blog.show_post', post_id=1) + '?reply=1', data=dict(author='Guest', email=
            'a@b.com', site='http://greyli.com', body='I am a guest comment', post=Post.query.get(1)), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Thanks, your comment will be published after reviewed', data)
        self.assertNotIn('I am a guest comment', data)
