...
class AdminTestCase(BaseTestCase):
    ...
    def test_new_post(self):
        ...
        response = self.client.post(url_for('admin.new_post'), data=dict(title='Something', category=1, body='Hello,world'),
    ...
    def test_edit_post(self):
        ...
        response = self.client.post(url_for('admin.edit_post', post_id=1), data=dict(title='Something Edited', category=1,
            body='New post body.'), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_approve_commet(self):
        self.logout()
        response = self.client.post(url_for('blog.show_post', post_id=1), data=dict(author='Guest', email='a@b.com', site=
            'http://greyli.com', body='I am a guest comment.', post=Post.query.get(1)), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_edit_category(self):
        response = self.client.post(url_for('admin.edit_category', category_id=1), data=dict(name='Default edited'),
                follow_redirects=True)
        data = response.get_data(as_text=True)
        ...
        response = self.client.post(url_for('admin.edit_category', category_id=2), data=dict(name='Life'), 
                follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_new_link(self):
        ...
        response = self.client.post(url_for('admin.new_link'), data=dict(name='HelloFlask', url='http://helloflask.com'),
                follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_edit_link(self):
        ...
        response = self.client.post(url_for('admin.edit_link', link_id=1), data=dict(name='Github', url='https://github.com/helloflask'), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_blog_setting(self):
        response = self.client.post(url_for('admin.settings'), data=dict(name='Grey Li', blog_title='My Blog',
            blog_sub_title='Just some raw ideas.', about='Example about page'), follow_redirects=True)
        data = response.get_data(as_text=True)
