...
def send_new_comment_email(post):
    ...
    send_mail(subject='New comment', to=current_app.config['BLUELOG_EMAIL'], html=' \
...
def send_new_reply_email(comment):
    ...
    send_mail(subject='New reply', to=comment.email,
            ...
            '<p><a href="%s">%s</a></p>'
            '<p><small style="color: #868e96">Do not reply this email.</small></p>' % (comment.post.title, post_url, post_url))
